...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  modifier onlyOwner { owner == msg.sender; _;}
  ...
  modifier forSale (uint sku) { items[sku].state == uint(State.ForSale); _;}
  modifier sold (uint sku){ items[sku].state == uint(State.Sold); _;}
  modifier shipped (uint sku) { items[sku].state == uint(State.Shipped); _;}
  modifier received (uint sku){ items[sku].state == uint(State.Received); _;}
